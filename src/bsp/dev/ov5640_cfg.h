...
#ifndef _OV5640_CFG_H_
#define _OV5640_CFG_H_

/* JPEG配置.7.5帧
   最大支持2592*1944的JPEG图像输出
 */
const unsigned short OV5640_JPEG_REG_TBL[][2] = {
    {0x4300, 0x30}, // YUV 422, YUYV
    {0x501f, 0x00}, // YUV 422
    /* Input clock = 24Mhz */
    {0x3036, 0x69}, // PLL  7.5fp
    // {0x3035, 0x21}, // PLL  3.8fp
    {0x3036, 0x35}, // PLL
    ...
    {0x460b, 0x35}, // MIPI global timing
    {0x460c, 0x22},
    {0x4837, 0x16},
    ...
    /* 将测量窗口分为4x4区域，每个区域有4位权重，最后的平均亮度是16个区域的加权平均值
       AEC是基于平均亮度调节的,当前增加了中心2x2区域权重
    */
    // {0x5688, 0x11}, // Bit[7-4]window 01 weight,Bit[3-0]window 00 weight
    // {0x5689, 0x11}, // Bit[7-4]window 03 weight,Bit[3-0]window 02 weight
    // {0x568A, 0x21}, // Bit[7-4]window 11 weight,Bit[3-0]window 10 weight
    // {0x568B, 0x12}, // Bit[7-4]window 13 weight,Bit[3-0]window 12 weight
    // {0x568C, 0x21}, // Bit[7-4]window 21 weight,Bit[3-0]window 20 weight
    // {0x568D, 0x12}, // Bit[7-4]window 23 weight,Bit[3-0]window 22 weight
    // {0x568E, 0x11}, // Bit[7-4]window 31 weight,Bit[3-0]window 30 weight
    // {0x568F, 0x11}, // Bit[7-4]window 33 weight,Bit[3-0]window 32 weight
    ...
    /* AEC target 曝光控制-3EU */
    // {0x3a0f, 0x10}, // stable range in high
    // {0x3a10, 0x08}, // stable range in low
    // {0x3a1b, 0x10}, // stable range out high
    // {0x3a1e, 0x08}, // stable range out low
    // {0x3a11, 0x20}, // fast zone high
    // {0x3a1f, 0x10}, // fast zone low
    /* AEC target 曝光控制-2EU */
    // {0x3a0f, 0x20}, // stable range in high
    // {0x3a10, 0x18}, // stable range in low
    // {0x3a1b, 0x41}, // stable range out high
    // {0x3a1e, 0x20}, // stable range out low
    // {0x3a11, 0x18}, // fast zone high
    // {0x3a1f, 0x10}, // fast zone low
    /* AEC target 曝光控制-2.5EU */
    // {0x3a0f, 0x18}, // stable range in high
    // {0x3a10, 0x10}, // stable range in low
    // {0x3a1b, 0x41}, // stable range out high
    // {0x3a1e, 0x10}, // stable range out low
    // {0x3a11, 0x10}, // fast zone high
    // {0x3a1f, 0x08}, // fast zone low
    /* AEC target 自动曝光控制-3EU */
    {0x3a0f, 0x10}, // stable range in high
    {0x3a10, 0x08}, // stable range in low
    {0x3a1b, 0x10}, // stable range out high
    {0x3a1e, 0x08}, // stable range out low
    {0x3a11, 0x00}, // fast zone high
    {0x3a1f, 0x10}, // fast zone low
};
/* EV曝光补偿设置参数表，支持7个等级 */
 ...
 */
const unsigned short OV5640_RGB565_REG_TBL[][2] = {
    {0x4300, 0X6F},
    {0X501F, 0x01},
    /* 1280x800, 15fps */
    ...
    {0x460b, 0x37}, // MIPI global timing
    {0x460c, 0x20},
    {0x4837, 0x16},
...
const unsigned short OV5640_UXGA_REG_TBL[][2] = {
...
/* EV曝光补偿设置参数表，支持7个等级 */
const unsigned char OV5640_EXPOSURE_TBL[7][6] = {
    {0x10, 0x08, 0x10, 0x08, 0x20, 0x10}, //-3
    // {0x18, 0x10, 0x41, 0x10, 0x10, 0x08},//-2 更改曝光阈值
    {0x20, 0x18, 0x41, 0x20, 0x18, 0x10}, //-2
    {0x30, 0x28, 0x61, 0x30, 0x28, 0x10}, //-1
    {0x38, 0x30, 0x61, 0x38, 0x30, 0x10}, // 0
    {0x40, 0x38, 0x71, 0x40, 0x38, 0x10}, //+1
...
/* 灯光模式参数表,支持5个模式 */
const unsigned char OV5640_LIGHTMODE_TBL[5][7] = {
...
/* 色彩饱和度设置参数表,支持7个等级 */
const unsigned char OV5640_SATURATION_TBL[7][6] = {
...
/* 特效设置参数表,支持7个特效 */
const unsigned char OV5640_EFFECTS_TBL[7][3] = {
    {0X06, 0x40, 0X10}, // 正常
    {0X1E, 0xA0, 0X40}, // 冷色
    {0X1E, 0x80, 0XC0}, // 暖色
    {0X1E, 0x80, 0X80}, // 黑白
